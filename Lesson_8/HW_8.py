...
"""Напишіть декоратор, який визначає час виконання функції. Заміряйте час віиконання функцій з попереднього ДЗ"""
...
def time_decorator(function_for_metering):

    def time_for_function(*args, **kwargs):
        start_time = timeit.default_timer()
        function_result = function_for_metering(*args, **kwargs)
        time_results = timeit.default_timer() - start_time
        print("Time of function execution:  ", time_results)
        return function_result

    return time_for_function
...
calculator(first_argument=7, second_argument=4.5, arithmetic_operation="*")
...
lib.seasons(data_for_check="11.11")
